...
# python lesions.py --mode predict -e isic2018/test_data/
...
# 181
        ...
        aug = iaa.SomeOf((0, 2), [iaa.Fliplr(0.5), iaa.Flipud(
            0.5), iaa.Affine(rotate=(-10, 10))])
...
# 188
        ...
        model.train(trainDataset, valDataset, epochs=20, layers="heads",
                    learning_rate=config.LEARNING_RATE, augmentation=aug)
        # pdb.set_trace() #mz
        # unfreeze the body of the network and train *all* layers
        model.train(trainDataset, valDataset, epochs=40, layers="all",
...
# 206
    ...
    # check to see if we are predicting using a trained Mask R-CNN
        ...
        for p in EXAMPLE_PATHS:
            ...
            # save image
            result_path = 'isic2018/test_data_labeled/'+p.split('/')[-1]
